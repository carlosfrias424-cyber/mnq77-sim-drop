...
Failed C2 → try again next 1m. 1m BODY close through the rail this visit
  = rail given = visit dead. Do not buy/sell the reclaim. Wick poke is ok.
...
class Pack:
    ...
    def close_through(self, close: float, bounce: bool) -> bool:
        # BODY vs the rail cluster — not the 10-pt arm zone.
        if bounce:
            return close < self.lo
        return close > self.hi
    ...
    for ln in follow(DEC):
        ...
        for pack in packs:
            ...
            if new_1m and closed is not None:
                ...
                if closed.c > pack.hi:
                    m.gave_up = True
                if (bounce and m.gave_dn) or ((not bounce) and m.gave_up):
                    m.visit_dead = True
                    m.reset_attempt()
                    rec["reason"] = "body_gave_rail"
                    rec["c1"] = dict(h=closed.h, l=closed.l, c=closed.c)
                    rec["gave_dn"] = m.gave_dn
                    rec["gave_up"] = m.gave_up
                    rec["visit_dead"] = True
                    emit(**rec)
                    continue
            ...
            if m.phase == "IDLE":
                ...
                rec["tape_lean"] = lean0
                rec["vol_ok"] = vol_ok
                if bounce and closed.c < pack.lo:
                    m.visit_dead = True
                    rec["reason"] = "body_gave_rail"
                    rec["c1"] = dict(h=closed.h, l=closed.l, c=closed.c)
                    rec["visit_dead"] = True
                    emit(**rec)
                    continue
                if (not bounce) and closed.c > pack.hi:
                    m.visit_dead = True
                    rec["reason"] = "body_gave_rail"
                    rec["c1"] = dict(h=closed.h, l=closed.l, c=closed.c)
                    rec["visit_dead"] = True
