...
def sr_kind(name: str) -> str | None:
    """Keep H4H vs H4L. Bare H4/H1 kept (side inferred from price)."""
...
def bounce_from_name(kind: str, px: float | None = None, mid: float | None = None) -> bool | None:
    u = (kind or "").upper()
    if u in SUPPORT:
        return True
    if u in RESIST:
        return False
    if u in BARE and px is not None and mid is not None:
        if mid > px:
            return True
        if mid < px:
            return False
...
def load_pois() -> list[Rail]:
    if not POI.exists():
        return []
    best = {}
    cut = session_open_ts() - RAIL_KEEP_S
...
def main():
    ...
    while True:
        ...
        mid_for_side = closed.c if closed is not None else last_px
        bounce = bounce_from_name(rail.kind, rail.px, mid_for_side)
        if bounce is None:
            rec["reason"] = "not_sr"
            emit(**rec)
            time.sleep(0.25)
