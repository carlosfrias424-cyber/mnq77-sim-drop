#!/usr/bin/env python3
"""Paper replay: Pine S/R + Databento 5m delta (same as live_77).
Support (H4L/H1L/PDL/PWL) = bounce long. Resistance (H4H/H1H/PDH/PWH) = fade short.
Wrong side = BRT skip. OPEN dropped. Cache trades if already pulled.
"""
...
def main():
    ...
    print("REPLAY DB", day0, "->", day1, "tape=databento_5m  sr_bounce_fade  paper 3/20/40")
    rails = load_rails(day0, day1)
    print("rails", len(rails), ":", ", ".join(f"{r.kind}@{r.px:.2f}" for r in sorted(rails, key=lambda x: -x.px)[:24]))
    ...
    for i, b in enumerate(rows):
        ...
        if bounce is None:
            reasons["not_sr"] += 1
            continue
        loc = loc_over(b["c"], rail.px)
        if loc is None:
            loc = loc_over(b["o"], rail.px)
        if loc is None:
            reasons["at_rail"] += 1
            continue
        if loc != bounce:
            visit_dead = True
            reasons["wrong_side_brt"] += 1
            continue
        ...
        rec = dict(
            ...
            poi=rail.key, sr="support" if bounce else "resistance",
            loc="over" if bounce else "under",
    ...
    print("\n=== PAPER FIRES (S/R + Databento 5m) ===")
    if not fires:
        print("none")
    else:
        print(f"{'when':<6} {'side':<4} {'entry':>8} {'poi':<22} {'hit':<5} {'MAE':>6} {'MFE':>6}")
        for f in fires:
            print(f"{f['t']:<6} {f['side']:<4} {f['entry']:8.2f} {f['poi']:<22} {f['hit']:<5} {f['mae']:6.1f} {f['mfe']:6.1f}")
